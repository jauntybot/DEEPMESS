...
public class UnitGameUI : UnitUI {
...
// Hard-coded portrait sprite placement <<.<<
        if (u is PlayerUnit) {
            ...
            portrait.rectTransform.localPosition = new Vector2(-100, 45);
            portrait.rectTransform.sizeDelta = new Vector2(200, 200);
            ...
            if (unit.equipment[i] is not MoveData) {
                ...
                if (b == null) {
                    ...
                    if (unit.equipment[i] is SlagGearData && unit.equipment[i] is not HammerData) {
                        ...
                        index = 1;
                    } else if (unit.equipment[i] is BeaconSignal) {
                        prefab = slagEquipmentButtonPrefab;
                        parent = equipmentParent.transform;
                    }
